...
func QueryById[T generic.ID](rc *redis.Client, db *gorm.DB, id T) (*User, error) {
	// 查缓存
	var obj User
	cacheKey := userKey.GetCacheUserById(id)
...
func QueryByEmail(rc *redis.Client, db *gorm.DB, email string) (*User, error) {
	// 查缓存
	var obj User
	cacheKey := userKey.GetCacheUserByEmail(email)
	...
	// 查数据库
	if err = db.Where("email = ?", email).Take(&obj).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			// 写空缓存（1分钟）
			if err := rc.Set(context.Background(), cacheKey, "", time.Minute).Err(); err != nil {
				xlog.Warn("空缓存写入失败，数据库有缓存穿透风险！" + err.Error())
